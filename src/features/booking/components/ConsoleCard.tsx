...
function levelTag(log: LogEntry): { text: string; cls: string } {
  switch (log.event ?? log.type) {
    ...
    default: return { text: "INFO", cls: "bg-slate-500/10 text-slate-500 dark:text-slate-300" };
  }
}
...
const STATUS_META: Record<TaskStatus, { icon: typeof Timer; iconCls: string; chip: string; title: string; titleCls: string }> = {
  idle: { icon: Timer, iconCls: "bg-slate-200 text-slate-500 dark:bg-white/10 dark:text-slate-300", chip: "text-slate-500 border-slate-300 bg-slate-100 dark:border-white/15 dark:bg-white/[0.06] dark:text-slate-300", title: "任务控制台", titleCls: "text-slate-800 dark:text-slate-100" },
    ...
    <GlassCard className="flex flex-col gap-3.5 p-4 md:p-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate font-mono text-[11.5px] text-slate-500 dark:text-slate-300">{subTitle}</div>
        </div>
        <span className={cn(
        ...
        <button
          ...
          aria-label="清空日志"
          onClick={() => setLogs([])}
          className="flex h-[30px] w-[30px] shrink-0 cursor-pointer items-center justify-center rounded-full border border-slate-300 bg-slate-100 text-slate-500 transition-colors hover:text-slate-700 dark:border-white/15 dark:bg-white/[0.07] dark:text-slate-300 dark:hover:text-slate-200"
        >
      ...
      <div>
                  ...
                  <div className={cn(
                    ...
                    isDone
                      ...
                      : isAct
                        ...
                        : "border-slate-300 bg-slate-100 text-slate-400 dark:border-white/15 dark:bg-white/[0.08] dark:text-slate-300",
                  )}>
                    {isDone ? <Check className="h-3.5 w-3.5" /> : isAct ? i + 1 : <Clock className="h-3.5 w-3.5" />}
                  </div>
                  <span className={cn(
                    "whitespace-nowrap text-[10px]",
                    isDone ? "text-green-600 dark:text-green-300" : isAct ? "font-bold text-blue-600 dark:text-blue-300" : "text-slate-400 dark:text-slate-300",
                  )}>{step.label}</span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={cn(
        ...
        <div className="mt-1.5 text-right font-mono text-[10.5px] text-slate-400 tabular-nums dark:text-slate-300">
      ...
      <div className="flex min-h-32 flex-col gap-1.5 rounded-2xl border border-slate-200 bg-slate-100/60 p-2.5 dark:border-white/[0.08] dark:bg-[#0a0c11]/60">
        {visibleLogs.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2.5 py-6">
            <EmptySeatsGlyph />
            <span className="text-[11.5px] text-slate-400 dark:text-slate-300">座位就绪，等待任务启动</span>
          </div>
        ) : (
          visibleLogs.map((log, i) => {
            ...
            return (
              ...
              >
                <span className="shrink-0 pt-px text-[10.5px] text-slate-400 dark:text-slate-300">{log.timestamp.split(" ")[0]}</span>
                ...
                <span className="min-w-0 flex-1 break-words text-slate-600 dark:text-slate-300">
                  ...
                  {log.count && log.count > 1 && (
                    <span className="ml-2 shrink-0 rounded-full bg-slate-300/70 px-1.5 py-px text-[9.5px] text-slate-500 dark:bg-white/10 dark:text-slate-300">×{log.count}</span>
                  )}
            ...
            <button
              ...
              className={cn(
                ...
                filter === f
                  ...
                  : "border-slate-300 bg-slate-100 text-slate-500 hover:text-slate-700 dark:border-white/10 dark:bg-white/[0.06] dark:text-slate-300 dark:hover:text-slate-200",
              )}
            >
